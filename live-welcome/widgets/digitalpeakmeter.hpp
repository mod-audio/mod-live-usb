...
protected:
    ...
    void paintEvent(QPaintEvent* const ev) override
    {
        ...
        int meterPad  = 0;
        int meterPos  = 0;
        int meterSize = (fMeterOrientation == HORIZONTAL ? height_ : width_)/fChannelCount;

        // set pen/brush for levels
        if (fMeterStyle == STYLE_OPENAV)
        {
            QColor colorTrans(fMeterColorBase);
            colorTrans.setAlphaF(0.5);
            painter.setBrush(colorTrans);
            painter.setPen(QPen(fMeterColorBase, 1));
            meterPad  += 2;
            meterSize -= 2;
        }
        else
        {
            painter.setPen(QPen(fMeterBackground, 0));
            painter.setBrush(fMeterGradient);
        }

        // draw levels
        for (int c=0; c<fChannelCount; ++c)
        {
            const float level = fChannelData[c];
            ...
            */
            ...
            if (fMeterOrientation == HORIZONTAL)
                painter.drawRect(0, meterPos, int(sqrtf(level) * float(width_)), meterSize);
            else if (fMeterOrientation == VERTICAL)
                painter.drawRect(meterPos, height_ - int(sqrtf(level) * float(height_)), meterSize, height_);
